...
describe('TaskService with Real Database', () => {
  ...
  describe('updateTaskUsage', () => {
    it('should accumulate usage values', async () => {
      ...
      await taskService.updateTaskUsage('task-usage', 0.01, 100, 50, 1, 40);
      await taskService.updateTaskUsage('task-usage', 0.02, 200, 100, 2, 60);
      ...
      const rows = db.rawQuery<{
        ...
        'SELECT cost, input_token, output_token, request_count, context_usage FROM conversations WHERE id = ?',
        ['task-usage']
      );
